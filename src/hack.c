...
moverock()
{
	...
	if (isok(rx,ry) && !IS_ROCK(levl[rx][ry].typ) &&
		...
		case TELEP_TRAP:
		    if (u.usteed) {
			pline("%s pushes %s and suddenly it disappears!", upstart(y_monnam(u.usteed)), the(xname(otmp)));
		    } else {
		...
		if (!u.usteed) {
		  ...
		  exercise(A_STR, TRUE);
		    ...
		    pline("%s moves %s.", upstart(y_monnam(u.usteed)), the(xname(otmp)));
		}
		lastmovetime = moves;
	    }
	...
	nopushmsg:
	  if (u.usteed) {
	    pline("%s tries to move %s, but cannot.", upstart(y_monnam(u.usteed)), the(xname(otmp)));
	  } else {
...
int mode;
{
	...
	if (closed_door(x,y)) {
		...
		if (mode == DO_MOVE) {
		    ...
		    else if (!Confusion && !Stunned && !Fumbling) {
...
domove()
{
	...
	if(u.utrap) {
		if(u.utraptype == TT_PIT && Passes_walls) {
			...
			if((u.utrap & 0xff) == 0) {
			    if (u.usteed) {
				You("lead %s to the edge of the lava.", y_monnam(u.usteed));
			    } else {
			...
			if (u.usteed) {
				pline("%s breaks out of the web.", upstart(y_monnam(u.usteed)));
			} else {
