...
STATIC_DCL int FDECL(generate_list_of_resistances, (char *, uchar));
...
lookat(int x, int y, char *buf, char *monbuf)
{
	...
	if (u.ux == x && u.uy == y && senseself()) {
		...
		Sprintf(buf, "%s%s%s called %s", Invis ? "invisible " : "", race, mons[u.umonnum].mname, plname);
		...
		if (mtmp != (struct monst *) 0) {
			...
			if (mtmp->mtrapped && cansee(mtmp->mx, mtmp->my)) {
				...
				if (tt == BEAR_TRAP || tt == PIT || tt == SPIKED_PIT || tt == WEB) {
					Sprintf(eos(buf), ", trapped in %s", an(defsyms[trap_to_defsym(tt)].explanation));
				}
			}

			{
				...
				if (mtmp->wormno ? worm_known(mtmp) : cansee(mtmp->mx, mtmp->my)) {
					if (mon_visible(mtmp) && !mtmp->minvis) {
						ways_seen++;
						normal++;
					}
				}
				...
				if (useemon && xraydist > 0) {
					if (distu(mtmp->mx, mtmp->my) <= xraydist) {
						ways_seen++;
					}
				}
				if (Detect_monsters) {
		...
		case S_altar:
			if(!In_endgame(&u.uz)) {
				Sprintf(buf, "%s altar", align_str(Amask2align(levl[x][y].altarmask & ~AM_SHRINE)));
			} else {
...
#define is_cmap_drawbridge(i) ((i) >= S_vodbridge && (i) <= S_hcdbridge)
		...
		for (hit_trap = FALSE, i = 0; i < MAXPCHARS; i++) {
			...
			if (sym == (force_defsyms ? defsyms[i].sym : (from_screen ? showsyms[i] : defsyms[i].sym)) && *x_str) {
				...
				if (!found) {
					...
					firstmatch = x_str;
					found++;
				} else if (!u.uswallow && !(hit_trap && is_cmap_trap(i)) && !(found >= 3 && is_cmap_drawbridge(i))) {
		...
		if (from_screen) {
			if (hallu_obj && Hallucination) {
				char temp_buf[BUFSZ];
				Sprintf(temp_buf, "%s (%s)", get_bogus_class_name(), get_bogus_item_name());
		...
		if (found) {
			...
			if (found == 1 && ans != LOOK_QUICK && ans != LOOK_ONCE) {
				if(ans == LOOK_VERBOSE || (flags.help && !quick)) {
					char temp_buf[BUFSZ];
...
int
generate_list_of_resistances(char * temp_buf, uchar flags)
{
	int many = 0;
...
get_flag_description_of_monster_type(struct permonst * ptr, char * description)
{
	...
	if (touch_disintegrates(ptr)) {
		strcat(description, " It can disintegrate by touch.");
	}
	if (vegan(ptr)) {
...
get_description_of_damage_type(uchar id)
{
	switch(id){
		...
		case AD_HEAD: return "decapitate";
		...
		default: impossible("bug in get_description_of_damage_type(%d)", id); return "<MISSING DECRIPTION, THIS IS A BUG>";
	}
}
...
*/
	...
	sprintf(temp_buf, "AC = %d, magic resistance = %d. ", ptr->ac, ptr->mr);
	strcat(description, temp_buf);
	...
	if (ignores_elbereth_and_scare_monster_scroll(ptr)) {
	...
	if (is_adult_dragon(ptr)) {
		if (!objects[monsndx(ptr)-PM_GRAY_DRAGON+GRAY_DRAGON_SCALES].oc_name_known) {
			if (!wizard || yn("Stop hiding adult dragons' attacks?") == 'n') {
				strcat(description, "Adult dragons' attacks are mysterious and hidden.");
				return description;
			}
		}
	}
