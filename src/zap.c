...
coord *cc;
{
	...
	if (mtmp2) {
		...
		if (!dmgtype(mtmp2->data, AD_SEDU) && !dmgtype(mtmp2->data, AD_SSEX)) mtmp2->mcan = 0;
...
struct obj *obj, *otmp;
{
	...
	case WAN_POLYMORPH:
	case SPE_POLYMORPH:
		if (is_unpolymorphable(obj) || obj_resists(obj, 5, 95)) {
		    res = 0;
		    break;
		}
		/* KMH, conduct */
...
struct obj *wand;
{
	if(wand->spe < 0 || (wand->spe == 0 && rn2(11)))
		return 0;
	if(wand->spe == 0)
...
struct obj **ootmp;	/* to return worn armor for caller to disintegrate */
{
	...
	*ootmp = (struct obj *)0;
	...
	case ZT_ACID:
		...
		tmp = d(nd,6);
		if (!rn2(6)) erode_obj(MON_WEP(mon),AD_ACID);
...
xchar sx, sy;
{
	...
	case ZT_ACID:
	    ...
	    if (!rn2(u.twoweap ? 3 : 6)) erode_obj(uwep, AD_ACID);
	    if (u.twoweap && !rn2(3)) erode_obj(uswapwep, AD_ACID);
