...
# endif
	nh_ND = Tgetstr("nd");
	if(tgetflag("os"))
		error("UnNetHackPlus can't have OS.");
	if(tgetflag("ul"))
		ul_hack = TRUE;
	CE = Tgetstr("ce");
	UP = Tgetstr("up");
...
/* not:		XD = Tgetstr("do"); */
	if(!(nh_CM = Tgetstr("cm"))) {
	    if(!UP && !HO)
		error("UnNetHackPlus needs CM or UP or HO.");
	    tty_raw_print("Playing UnNetHackPlus on terminals without CM is suspect.");
	    tty_wait_synch();
	}
	SO = Tgetstr("so");
	SE = Tgetstr("se");
